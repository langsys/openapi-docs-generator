...
// --- Folder and Request Ordering ---

test('sorts folders by OpenAPI tag order', function () {
    writeOpenApi($this->openApiFile, [
        'tags' => [
            ['name' => 'Auth'],
            ['name' => 'Users'],
    ...
    // Folders should follow the tag order: Auth, Users, Projects
    expect($folderNames)->toBe(['Auth', 'Users', 'Projects']);
});

test('sorts requests grouped by folder order', function () {
    writeOpenApi($this->openApiFile, [
        'tags' => [
            ['name' => 'Auth'],
            ['name' => 'Users'],
    ...
    $col = json_decode(file_get_contents($this->collectionFile), true);
    $requestNames = array_column($col['requests'], 'name');

    // Auth requests should come before Users requests
    expect($requestNames)->toBe(['Login', 'List Users']);
});

test('folders without tag order go to end', function () {
    writeOpenApi($this->openApiFile, [
        'tags' => [
            ['name' => 'Auth'],
        ],
    ...
    // Auth is in tags array, Users is not -> Auth first, Users last
    expect($folderNames[0])->toBe('Auth');
    expect($folderNames[1])->toBe('Users');
});
