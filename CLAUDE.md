...
- **Generators\ErrorContract** — Reads and validates an error class's identity (see **Error Class Contract**): `isErrorClass()` (non-abstract subclass of `errors.base_class`) and `read()` → `{code, message, status, markers}`. Validates `errors.base_class` exists and extends Spatie `Data`. Owned by `DtoSchemaBuilder` (`getErrorContract()`); the generator passes it to the attacher so failures can say whether a class is not an error class or was never scanned.
- **Generators\ErrorEnvelope** — The one definition of the error response shape: `{ status, data, error: { message, code, template, params, details, ...#[EnvelopeField] props } }`. Names from `errors.response_fields` / `errors.error_fields` (`null` omits; `error` and `code` can't be null; unknown keys, duplicate names and the removed `errors.fields` key throw). Builds `{Name}Body` (`message` example = `MESSAGE` with markers filled from `#[Example]` values, `template` = `MESSAGE`, `params` from the markers, `code` single-value enum, `details` `$ref`; throws if an `#[EnvelopeField]` reuses an error-object name), `{Name}Response`, and `sharedStatusSchema()` for the attacher, which puts the `oneOf` + code discriminator on the `error` property over the Body schemas because OAS 3.0 discriminators need a top-level property of each variant. Schemas stay open (no `additionalProperties: false`) so undocumented fields like app debug output validate. The builder owns the instance (`getErrorEnvelope()`) and the generator passes it to the attacher, so attached responses always match the schemas they reference.
- **Generators\OperationErrorAttacher** — Attaches error responses to operations. Sources, unioned and deduped by class: `#[Throws]` on the action (or its controller class), `implied_errors.middleware` (matched via `MiddlewareFilter` against the route's fully-resolved middleware), two structural rules (`implied_errors.validation` when the action takes a Spatie `Data` parameter; `implied_errors.not_found` when the route has a bound `{param}` — `not_found_binding` 'model' (default, implicit model binding or `Route::bind()`) or 'any'), and app-supplied `implied_errors.rules`. Grouped by status: one error → `$ref` to `components.responses.{Name}`; several → inline response from `ErrorEnvelope::sharedStatusSchema()`, a `oneOf` of their `{Name}Body` schemas on the `error` property discriminated on the configured code field, **plus one `OA\Examples` per error** on the media type (keyed and summarised by the bare code for a switcher's list, `description` repeating that error's `` `code`: MESSAGE `` line so example metadata names the error on screen, ordered as the description's list): a bare `oneOf` renders one synthesized body that reads as the only answer. Each value is a whole envelope from `ErrorEnvelope::exampleEnvelope()`, whose error object comes from the examples the error's serialized `{Name}Body`/details schemas already advertise (`indexSchemas()` + `exampleFromSchema()`, ref-cycle safe); an array takes a one-item list from its `items` example (where a scalar `#[Example]` on an array lands), and a property with no example anywhere is omitted rather than invented, so an app-owned `#[EnvelopeField]` never shows fabricated content, and `ErrorEnvelope::exampleErrorObject()` is the fallback when the Body schema isn't in the document. Hand-written responses for a status win. **Validation scenarios**: when a resolver reports scenarios, that operation's validation response turns inline even for a single error (the `$ref` points at a component every endpoint shares), keeping the error's `{Name}Response` schema and appending `Possible validation errors:` with one `` `field`.`code`: message `` line each (fieldless ones as `` `code`: message ``); a shared status keeps its `oneOf` and gains the same block; scenarios are deduped on the exact field/code/message triple (not field+code: several rules legitimately share a code with different messages), first wins. Throws `OpenApiDocsException` when a declared class is not a concrete subclass of `errors.base_class` or was never scanned (the precise message needs the `ErrorContract`, which the generator passes). When an @OA annotation sits on a helper, rules and the Data-parameter and binding checks inspect the matched route's real action (`ResolvedRoute::action()`), while `#[Throws]` is read from both methods. Inert without `#[Throws]`/`implied_errors`; without a `RouteResolver` only `#[Throws]` and the validation rule apply.
- **Contracts\ValidationScenarioResolver** — App-owned resolver: `scenariosFor(OperationContext, ?ReflectionMethod): Data\ValidationScenario[]` (a nullable `field`, a `code` and a `message`; the constructor rejects empty values, and a null field means the rule validates the whole payload). Config `implied_errors.validation_scenarios`, same descriptor forms as `rules`, built by **Generators\ValidationScenarioResolverFactory** (which also accepts a single descriptor, not just a list). Requires `implied_errors.validation`: scenarios are listed on that error's response, so returning them without it throws, naming the operation. A resolver returning a non-`ValidationScenario` throws too.
...
- **Message templates.** `MESSAGE` may contain `{marker}` placeholders (`ErrorContract::MARKER_PATTERN`, `/\{([a-z][a-z0-9_]*)\}/`). Each must name a public, non-static property of the class; `ErrorContract::read()` returns them as `markers`. `{Name}Body` gets `template` (`MESSAGE` verbatim, required) and `params` (an object of the marker properties, built like DTO properties, required, omitted when `MESSAGE` has no markers). The `message` example substitutes each marker's `#[Example]` value. `errors.error_fields.template`/`params` rename them, or omit them with `null`.
- Hard failures (`OpenApiDocsException`, before writing): a `MESSAGE` `{marker}` naming no public property, a missing or inherited `CODE`/`MESSAGE`, an empty value, a duplicate `CODE`, an unresolvable or out-of-range `STATUS`, a class-level `#[Description]`, an `#[EnvelopeField]` reusing an error-object field name, and a non-existent or non-Data `errors.base_class`.
...
Tests use Pest with Orchestra Testbench (285 tests, 832 assertions).
...
| `tests/Unit/ErrorSchemaBuilderTest.php` | Base-class discovery, constant reading (int/enum `STATUS`, inheritance from abstract and concrete parents), contract violations generated into temp dirs (missing/inherited/empty `CODE`/`MESSAGE`, `STATUS`, class `Description`, duplicate code, envelope-field collision), base-class validation, details/Body/Response shapes, `buildErrorCodeSchema()` subsets, field config + validation, `errors.paths`, message templates (`template`/`params` from `{marker}`s, filled `message` example, marker without a public property, `template`/`params` envelope-field collisions, null omission) |
...
| `tests/Unit/OperationErrorAttacherTest.php` | Shared-status examples (one per error, keyed/ordered by code, whole envelope values from the Body/details schemas, app-owned envelope field omitted, definition-only fallback, none on single-error responses, wrapper label), Validation scenarios (inline response + description lines, nested dotted and fieldless fields, exact-triple dedupe keeping one code with different messages, shared status, hand-written precedence, missing `validation` config, wrong return type, descriptor forms, `ValidationScenario` validation), `$ref` vs `error`-level `oneOf`+discriminator (incl. configured field names), annotation precedence, middleware/validation/not-found rules, dedupe, class-level `Throws`, no-resolver and no-context cases, both failure messages, custom `ImpliedErrorRule` descriptors + contract errors, rules and the validation check using the matched route's action when the annotation sits on a helper |
